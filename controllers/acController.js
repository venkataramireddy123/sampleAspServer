...
module.exports = function (app, pool) {
    app.get("/api/acs/GetAll", function (req, res) {
    ...
    app.post("/api/acs/GetAllByBoardId", function (req, res) {
        commonMethods.parseToken(pool, req, res, function () {
            pool.getConnection(function (err, connection) {
                // connected! (unless `err` is set)
                if (err) {
                ...
                connection.query('SELECT * FROM hau_acs where (AC_ISDELETED  is null or AC_ISDELETED = 0) and AC_BORD_ID = ' + req.body.AC_BORD_ID, function (error, results, fields) {
                    // And done with the connection.
                    connection.release();
    ...
    app.get("/api/acs/GetByID", function (req, res) {
        commonMethods.parseToken(pool, req, res, function () {
            pool.getConnection(function (err, connection) {
                // connected! (unless `err` is set)
                if (err) {
                ...
                connection.query('SELECT * FROM hau_acs where AC_ID = ' + req.query['id'] + ';SELECT * FROM hau_acshistory where ACH_AC_ID = ' + req.query['id'] + ' AND (ACH_CREATEDDATE BETWEEN DATE_SUB(curdate(), INTERVAL 1 YEAR) AND current_timestamp());SELECT * FROM hau_acswitches ls inner join hau_switchs s on s.SWIT_ID = ls.ACSW_SWIT_ID inner join hau_boards b on b.BORD_ID = s.SWIT_BORD_ID inner join hau_rooms r on r.ROOM_ID = b.BORD_ROOM_ID WHERE ACSW_AC_ID = ' + req.query['id'] + '; SELECT * FROM hau_scheduling s left outer join hau_appliances a on a.APPL_ID = s.TUSH_DEV_ID left outer join hau_acs l on a.APPL_ID = l.AC_APPL_ID left outer join hau_switchs sw on sw.SWIT_ID = s.TUSH_SWIT_ID left outer join hau_sensors sen on sen.SR_ID = s.TUSH_SR_ID left outer join hau_sensortypes senty on senty.SRTY_ID = sen.SR_SRTY_ID where l.AC_ID = ' + req.query['id'] + ';', function (error, results, fields) {
                    // And done with the connection.
                    connection.release();
    ...
    app.post("/api/acs/postData", function (req, res) {
        commonMethods.parseToken(pool, req, res, function () {
            pool.getConnection(function (err, connection) {
                // connected! (unless `err` is set)
                if (err) {
                ...
                if (req.body["AC_ID"]) {
                    connection.query("SELECT * FROM hau_acs where (AC_ISDELETED  is null or AC_ISDELETED = 0) and AC_ID <>" + req.body.AC_ID + " and AC_PIN = " + req.body.AC_PIN + " and AC_BORD_ID =" + req.body.AC_BORD_ID + " ;", function (error1, results1, fields1) {
                        if (error1) {
                            res.send({
                                'ResultCode': 202,
                                'details': undefined,
                    ...
                    req.body.AC_CREATEDDATE = commonMethods.GetDateToString(req.body.AC_CREATEDDATE);
                    connection.query("SELECT * FROM hau_acs where (AC_ISDELETED  is null or AC_ISDELETED = 0) and AC_PIN = " + req.body.AC_PIN + " and AC_BORD_ID =" + req.body.AC_BORD_ID + " ;", function (error1, results1, fields1) {
                        if (error1) {
                            res.send({
                                'ResultCode': 202,
                                'details': undefined,
                        ...
                        if (results1.length == 0) {
                            connection.query('INSERT INTO hau_appliances SET ?', { "APPL_TYPE": 5, "APPL_BORD_ID": req.body.AC_BORD_ID, "APPL_CREATEDBY": req.body.AC_CREATEDBY, "APPL_CREATEDDATE": req.body.AC_CREATEDDATE }, function (_error, _results, _fields) {
                                ...
                                req.body.AC_APPL_ID = _results.insertId;

                                connection.query('INSERT INTO hau_acs SET ?', req.body, function (error, results, fields) {
                                    // And done with the connection.
                                    connection.release();
