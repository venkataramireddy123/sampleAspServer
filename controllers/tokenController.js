...
module.exports = function (app, pool) {
    app.get("/api/token/GetAll", function (req, res) {
    ...
    app.get("/api/token/GetByID", function (req, res) {
        pool.getConnection(function (err, connection) {
            // connected! (unless `err` is set)
            if (err) { res.send({ 'ResultCode': 202, 'details': undefined, 'ResultText': "No Network found!" }); return;};

            connection.query('SELECT * FROM hau_token where TOKN_ID = ' + req.query['id'], function (error, results, fields) {
                // And done with the connection.
                connection.release();
    ...
    app.post("/api/token/postData", function (req, res) {
        pool.getConnection(function (err, connection) {
            // connected! (unless `err` is set)
            if (err) { res.send({ 'ResultCode': 202, 'details': undefined, 'ResultText': "No Network found!" }); return;};
            var obj = req.body;
            ...
            if (req.body["TOKN_ID"]) {
                ...
                connection.query('INSERT INTO hau_token SET ?', req.body, function (error, results, fields) {
                    // And done with the connection.
                    connection.release();
