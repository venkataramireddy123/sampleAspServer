...
module.exports = function (app, pool) {
    app.get("/api/tvs/GetAll", function (req, res) {
    ...
    app.post("/api/tvs/GetAllByBoardId", function (req, res) {
        commonMethods.parseToken(pool, req, res, function () {
            pool.getConnection(function (err, connection) {
                // connected! (unless `err` is set)
                if (err) {
                ...
                connection.query('SELECT * FROM hau_tvs where (TV_ISDELETED  is null or TV_ISDELETED = 0) and TV_BORD_ID = ' + req.body.TV_BORD_ID, function (error, results, fields) {
                    // And done with the connection.
                    connection.release();
    ...
    app.get("/api/tvs/GetByID", function (req, res) {
        commonMethods.parseToken(pool, req, res, function () {
            pool.getConnection(function (err, connection) {
                // connected! (unless `err` is set)
                if (err) {
                ...
                connection.query('SELECT * FROM hau_tvs where TV_ID = ' + req.query['id'] + ';SELECT * FROM hau_tvshistory where TVH_TV_ID = ' + req.query['id'] + ' AND (TVH_CREATEDDATE BETWEEN DATE_SUB(curdate(), INTERVAL 1 YEAR) AND current_timestamp());SELECT * FROM hau_tvswitches ls inner join hau_switchs s on s.SWIT_ID = ls.TVSW_SWIT_ID inner join hau_boards b on b.BORD_ID = s.SWIT_BORD_ID inner join hau_rooms r on r.ROOM_ID = b.BORD_ROOM_ID WHERE TVSW_TV_ID = ' + req.query['id'] + '; SELECT * FROM hau_scheduling s left outer join hau_appliances a on a.APPL_ID = s.TUSH_DEV_ID left outer join hau_tvs l on a.APPL_ID = l.TV_APPL_ID left outer join hau_switchs sw on sw.SWIT_ID = s.TUSH_SWIT_ID left outer join hau_sensors sen on sen.SR_ID = s.TUSH_SR_ID left outer join hau_sensortypes senty on senty.SRTY_ID = sen.SR_SRTY_ID where l.TV_ID = ' + req.query['id'] + ';', function (error, results, fields) {
                    // And done with the connection.
                    connection.release();
    ...
    app.post("/api/tvs/postData", function (req, res) {
        commonMethods.parseToken(pool, req, res, function () {
            pool.getConnection(function (err, connection) {
                // connected! (unless `err` is set)
                if (err) {
                ...
                if (req.body["TV_ID"]) {
                    connection.query("SELECT * FROM hau_tvs where (TV_ISDELETED  is null or TV_ISDELETED = 0) and TV_ID <>" + req.body.TV_ID + " and TV_PIN = " + req.body.TV_PIN + " and TV_BORD_ID =" + req.body.TV_BORD_ID + " ;", function (error1, results1, fields1) {
                        if (error1) {
                            res.send({
                                'ResultCode': 202,
                                'details': undefined,
                    ...
                    req.body.TV_CREATEDDATE = commonMethods.GetDateToString(req.body.TV_CREATEDDATE);
                    connection.query("SELECT * FROM hau_tvs where (TV_ISDELETED  is null or TV_ISDELETED = 0) and TV_PIN = " + req.body.TV_PIN + " and TV_BORD_ID =" + req.body.TV_BORD_ID + " ;", function (error1, results1, fields1) {
                        if (error1) {
                            res.send({
                                'ResultCode': 202,
                                'details': undefined,
                        ...
                        if (results1.length == 0) {
                            connection.query('INSERT INTO hau_appliances SET ?', { "APPL_TYPE": 4, "APPL_BORD_ID": req.body.TV_BORD_ID, "APPL_CREATEDBY": req.body.TV_CREATEDBY, "APPL_CREATEDDATE": req.body.TV_CREATEDDATE }, function (_error, _results, _fields) {
                                ...
                                req.body.TV_APPL_ID = _results.insertId;

                                connection.query('INSERT INTO hau_tvs SET ?', req.body, function (error, results, fields) {
                                    // And done with the connection.
                                    connection.release();
